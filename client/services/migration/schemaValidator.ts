import { GraphNode, NodeType, EmbeddedEdge, ChatMessage, NodeColor } from '../../types';
import { extractFirstNounPhrase, cleanTitleMarkdown } from '../../utils/titleUtils';
import { SchemaValidationResult } from './types';

const VALID_NODE_TYPES = Object.values(NodeType);
const VALID_COLORS: NodeColor[] = ['slate', 'red', 'green', 'blue', 'amber', 'purple'];
...
export function validateNodeSchema(node: GraphNode): SchemaValidationResult {
  ...
  if (!node.title || node.title.trim() === '') {
    ...
    invalidFields.push({ field: 'title', reason: 'Title missing, generated from content' });
  } else {
    ...
    if (cleanedTitle !== node.title) {
      ...
      invalidFields.push({ field: 'title', reason: 'Title contained markdown formatting' });
    }
  }

  // Optional field type validation
  if (node.width !== undefined && (typeof node.width !== 'number' || node.width <= 0)) {
    invalidFields.push({ field: 'width', reason: 'width must be a positive number' });
    suggestedFixes.width = 300;
  }

  if (node.height !== undefined && (typeof node.height !== 'number' || node.height <= 0)) {
    invalidFields.push({ field: 'height', reason: 'height must be a positive number' });
    suggestedFixes.height = 200;
  }

  if (node.color !== undefined && !VALID_COLORS.includes(node.color)) {
    invalidFields.push({ field: 'color', reason: `Invalid color: ${node.color}` });
  ...
  if (node.edges !== undefined) {
    if (!Array.isArray(node.edges)) {
      ...
      if (validEdges.length !== node.edges.length) {
        invalidFields.push({ field: 'edges', reason: 'Some edges have invalid structure' });
  ...
  if (node.type === NodeType.CHAT && node.messages !== undefined) {
    if (!Array.isArray(node.messages)) {
      invalidFields.push({ field: 'messages', reason: 'messages must be an array' });
      ...
      if (validMessages.length !== node.messages.length) {
        invalidFields.push({ field: 'messages', reason: 'Some messages have invalid structure' });
  ...
  if (node.aliases !== undefined) {
    if (!Array.isArray(node.aliases)) {
      invalidFields.push({ field: 'aliases', reason: 'aliases must be an array' });
      suggestedFixes.aliases = [];
    } else if (!node.aliases.every((a) => typeof a === 'string')) {
      invalidFields.push({ field: 'aliases', reason: 'aliases must be strings' });
      suggestedFixes.aliases = node.aliases.filter((a) => typeof a === 'string');
    }
  }
  ...
  if (node.clusterIds !== undefined) {
    if (!Array.isArray(node.clusterIds)) {
      invalidFields.push({ field: 'clusterIds', reason: 'clusterIds must be an array' });
      suggestedFixes.clusterIds = [];
    } else if (!node.clusterIds.every((id) => typeof id === 'string')) {
      invalidFields.push({ field: 'clusterIds', reason: 'clusterIds must be strings' });
      suggestedFixes.clusterIds = node.clusterIds.filter((id) => typeof id === 'string');
    }
  }

  // autoExpandDepth validation
  if (node.autoExpandDepth !== undefined && typeof node.autoExpandDepth !== 'number') {
    invalidFields.push({ field: 'autoExpandDepth', reason: 'autoExpandDepth must be a number' });
    suggestedFixes.autoExpandDepth = 0;
  }

  // clusterCount validation
  if (node.clusterCount !== undefined && typeof node.clusterCount !== 'number') {
    invalidFields.push({ field: 'clusterCount', reason: 'clusterCount must be a number' });
...
function generateTitleFromContent(content: string): string {
  return extractFirstNounPhrase(content, 30);
}
