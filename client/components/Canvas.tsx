...
import { GraphEdge, GraphNode, NodeType, ViewportTransform, ResizeDirection, ChatMessage, LODLevel } from '../types';
import { GraphNodeComponent } from './GraphNode';
import { Edge } from './Edge';
import { SkeletonGraph, NodeSkeleton } from './SkeletonGraph';
import { DEFAULT_NODE_HEIGHT, DEFAULT_NODE_WIDTH, MIN_NODE_WIDTH, MIN_NODE_HEIGHT, COLORS, NODE_COLORS } from '../constants';
import { sendChatMessage, getTopicSummaryPrompt } from '../services/geminiService';
...
export const Canvas: React.FC<CanvasProps> = ({ 
  ...
  useEffect(() => { connectingNodeIdRef.current = connectingNodeId; }, [connectingNodeId]);
  ...
  const { visibleNodes, bufferedNodes, visibleEdges, lodLevel } = useMemo(() => {
    ...
    quadtree.visit((node, x1, y1, x2, y2) => {
        ...
        if (!node.length) {
            do {
                ...
                if (d.x < renderRect.right && right > renderRect.left &&
                    d.y < renderRect.bottom && bottom > renderRect.top) {
                    visible.push(d);
                }
            } while (node = node.next);
        }
        return x1 >= renderRect.right || y1 >= renderRect.bottom || x2 < queryLeft || y2 < queryTop;
    });
    ...
    if (currentLod !== 'CLUSTER') {
        visEdges = edges.filter(e => {
            ...
            return (
              left < renderRect.right && right > renderRect.left &&
              top < renderRect.bottom && bottom > renderRect.top
            );
        });
    }
  ...
  useEffect(() => {
    ...
    const zoom = d3.zoom<HTMLDivElement, unknown>()
      ...
      .on('zoom', (event) => {
        ...
        if (t.k < LOD_THRESHOLD_SEMANTIC_SHIFT && onNavigateUp && currentScopeId) {
      ...
      .filter((event) => {
        if (draggingIdRef.current || resizingIdRef.current || connectingNodeIdRef.current) return false;
    ...
    selection.on("wheel.pan", (event) => {
       ...
       zoom.translateBy(selection, -event.deltaX / currentK, -event.deltaY / currentK);
    });
  ...
  useEffect(() => {
    ...
    if (kDiff > 0.001 || xDiff > 0.1 || yDiff > 0.1) {
       selection.call(zoomBehaviorRef.current.transform, d3.zoomIdentity.translate(viewTransform.x, viewTransform.y).scale(viewTransform.k));
    }
  }, [viewTransform]);

  const resolveCollisions = useCallback((fixedNodeId?: string) => {
    setNodes(currentNodes => {
      ...
      return currentNodes.map((n, i) => ({ ...n, x: (simNodes[i] as any).x, y: (simNodes[i] as any).y }));
    });
  }, [setNodes, edges]);
  ...
  const handleCreateFromSelection = useCallback(async (type: NodeType) => {
      ...
      const initialMessages: ChatMessage[] = type === NodeType.CHAT ? [
          { role: 'user', text: selectionTooltip.text, timestamp: Date.now() }
      ] : [];
      const initialModelMsg: ChatMessage | undefined = type === NodeType.CHAT 
        ? { role: 'model', text: '', timestamp: Date.now() } : undefined;
      const startMessages = type === NodeType.CHAT && initialModelMsg ? [...initialMessages, initialModelMsg] : initialMessages;

      const newNode: GraphNode = {
        id: crypto.randomUUID(), type, x: newNodeX, y: newNodeY,
        content: type === NodeType.NOTE ? selectionTooltip.text : (selectionTooltip.text.length > 30 ? selectionTooltip.text.substring(0, 30) + '...' : selectionTooltip.text),
        messages: startMessages, width: DEFAULT_NODE_WIDTH, height: DEFAULT_NODE_HEIGHT,
      ...
      if (selectionTooltip.sourceId) {
         const labelText = selectionTooltip.text.length > 20 ? selectionTooltip.text.substring(0, 20) + '...' : selectionTooltip.text;
         setEdges(prev => [...prev, { id: crypto.randomUUID(), source: selectionTooltip.sourceId!, target: newNode.id, label: labelText, parentId: currentScopeId || undefined }]);
      }
      ...
      if (type === NodeType.CHAT) {
         try {
             ...
             const result = await sendChatMessage([], promptTemplate, (chunk) => {
                currentText += chunk;
                setNodes(prev => prev.map(n => n.id === newNode.id && n.messages ? { ...n, messages: [...n.messages.slice(0, -1), { ...n.messages[n.messages.length-1], text: currentText }] } : n));
             });
             setNodes(prev => prev.map(n => n.id === newNode.id && n.messages ? { ...n, messages: [...n.messages.slice(0, -1), { ...n.messages[n.messages.length-1], text: result.text }] } : n));
         } catch (e) {
             console.error("Failed to generate initial response", e);
         }
      }
  }, [selectionTooltip, nodes, viewTransform, setNodes, setEdges, currentScopeId, onNodeSelect]);

  // Selection Listeners
  useEffect(() => {
    const handleSelectionChange = () => {
       const sel = window.getSelection();
       if ((!sel || sel.isCollapsed) && document.activeElement?.tagName !== 'TEXTAREA') {
         setSelectionTooltip(null);
       }
    };
    const handleMouseUp = (e: MouseEvent | TouchEvent) => {
      if (draggingId || resizingId || dragStartRef.current || connectingNodeId) return; 
      const target = e.target as HTMLElement;
      if (target.closest('.selection-tooltip')) return;

      let text = '', rect: { left: number, top: number, width: number, height: number, bottom?: number } | null = null, sourceId: string | undefined;
      ...
      if (activeEl && activeEl.tagName === 'TEXTAREA') {
         ...
         if (start !== end) {
            ...
            let clientX = 'touches' in e ? (e as TouchEvent).changedTouches[0].clientX : (e as MouseEvent).clientX;
            let clientY = 'touches' in e ? (e as TouchEvent).changedTouches[0].clientY : (e as MouseEvent).clientY;
            rect = { left: clientX, top: clientY - 40, width: 0, height: 0, bottom: clientY + 10 };
         ...
         if (selection && !selection.isCollapsed) {
            ...
            if (text.trim()) {
                ...
                while(curr && (curr.nodeType !== Node.ELEMENT_NODE || !(curr as HTMLElement).dataset.nodeId)) {
  ...
  useEffect(() => {
    const handleEnd = () => {
      ...
      setDraggingId(null); setResizingId(null); setResizeDirection(null); dragStartRef.current = null;
    ...
    const handleMove = (e: MouseEvent | TouchEvent) => {
      ...
      if (e.type === 'touchmove' && (draggingId || resizingId)) e.preventDefault();

      const { mouseX, mouseY, nodeX, nodeY, nodeWidth, nodeHeight } = dragStartRef.current;
      const dx = (clientX - mouseX) / viewTransform.k;
      const dy = (clientY - mouseY) / viewTransform.k;

      if (draggingId) {
        setNodes((prev) => prev.map((n) => n.id === draggingId ? { ...n, x: nodeX + dx, y: nodeY + dy } : n));
      } else if (resizingId && resizeDirection) {
        setNodes((prev) => prev.map((n) => {
            if (n.id === resizingId) {
              let newX = nodeX, newY = nodeY, newW = nodeWidth, newH = nodeHeight;
              if (resizeDirection.includes('e')) newW = Math.max(MIN_NODE_WIDTH, nodeWidth + dx);
              else if (resizeDirection.includes('w')) {
                const effectiveDx = Math.min(dx, nodeWidth - MIN_NODE_WIDTH);
                newW = nodeWidth - effectiveDx;
                newX = nodeX + effectiveDx;
              }
              if (resizeDirection.includes('s')) newH = Math.max(MIN_NODE_HEIGHT, nodeHeight + dy);
  ...
  }, [draggingId, resizingId, resizeDirection, viewTransform.k, setNodes, resolveCollisions]);

  const handleNodeMouseDown = useCallback((e: React.MouseEvent | React.TouchEvent, id: string) => {
    e.stopPropagation();
    if (connectingNodeId) {
    ...
    const target = e.target as HTMLElement;
    if (['INPUT', 'TEXTAREA'].includes(target.tagName) || target.closest('button') || target.closest('a')) return;
    ...
    setDraggingId(id);
    dragStartRef.current = { mouseX: clientX, mouseY: clientY, nodeX: node.x, nodeY: node.y, nodeWidth: node.width || DEFAULT_NODE_WIDTH, nodeHeight: node.height || DEFAULT_NODE_HEIGHT };
  ...
  const handleBackgroundClick = useCallback((e: React.MouseEvent | React.TouchEvent) => {
     ...
     if (target === containerRef.current || target.closest('.canvas-background')) {
        onNodeSelect(null);
        setSelectionTooltip(null);
     }
  }, [connectingNodeId, onCancelConnect, onNodeSelect]);

  const handleResizeStart = useCallback((e: React.MouseEvent | React.TouchEvent, id: string, direction: ResizeDirection) => {
    e.stopPropagation();
    const node = nodes.find(n => n.id === id);
    if (!node) return;
    let clientX = 'touches' in e ? e.touches[0].clientX : (e as React.MouseEvent).clientX;
  ...
  const applyLayout = (type: 'force' | 'tree-tb' | 'tree-lr') => {
      ...
      if (type === 'force') {
        d3.forceSimulation(simNodes as any)
            .force("charge", d3.forceManyBody().strength(-2000))
            .force("link", d3.forceLink(simEdges).id((d: any) => d.id).distance(500)) 
            .force("center", d3.forceCenter(0, 0))
            .force("collide", d3.forceCollide().radius(200)) 
            .stop().tick(300);
        setNodes(prev => prev.map((n, i) => ({ ...n, x: (simNodes[i] as any).x, y: (simNodes[i] as any).y })));
      } else {
        const rootId = nodes[0].id; 
        const stratify = d3.stratify<GraphNode>().id(d => d.id).parentId(d => {
        ...
        try {
            ...
            const treeLayout = d3.tree<GraphNode>().nodeSize(type === 'tree-tb' ? [320, 220] : [320, 250]);
            treeLayout(root);
            const descendants = root.descendants();
            setNodes(prev => prev.map(n => {
                const d = descendants.find(dn => dn.id === n.id);
                return d ? { ...n, x: type === 'tree-tb' ? d.x : d.y, y: type === 'tree-tb' ? d.y : d.x } : n;
            }));
        } catch(e) { console.warn(e); applyLayout('force'); }
      }
  };
  
  const addNewNode = (type: NodeType) => {
    const cx = (containerSize.width / 2 - viewTransform.x) / viewTransform.k - DEFAULT_NODE_WIDTH / 2;
    const cy = (containerSize.height / 2 - viewTransform.y) / viewTransform.k - DEFAULT_NODE_HEIGHT / 2;
    const newNode: GraphNode = {
      id: crypto.randomUUID(), type, x: cx, y: cy,
      content: type === NodeType.CHAT ? 'New Chat' : '', width: DEFAULT_NODE_WIDTH, height: DEFAULT_NODE_HEIGHT, messages: [],
      parentId: currentScopeId || undefined
    };
    setNodes(prev => [...prev, newNode]);
    onNodeSelect(newNode.id);
      ...
      if (source) {
          ...
          connectingLine = (
              <line 
                ...
                stroke={COLORS.activeEdgeStroke} strokeWidth={2} strokeDasharray="5,5"
  ...
  return (
    <div className="flex flex-col md:flex-row w-full h-full overflow-hidden bg-slate-950">
      {/* Toolbar */}
      <div className="z-40 bg-slate-900 border-b md:border-b-0 md:border-r border-slate-800 shadow-xl w-full h-14 md:w-16 md:h-full flex flex-row md:flex-col items-center justify-between p-2 md:py-4 shrink-0">
        <div className="flex flex-row md:flex-col gap-2 md:gap-4">
           <button onClick={onToggleMenu} className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center shadow-lg mb-0 md:mb-4 hover:brightness-110 transition-all group">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="group-hover:scale-110 transition-transform"><line x1="4" y1="12" x2="20" y2="12"/><line x1="4" y1="6" x2="20" y2="6"/><line x1="4" y1="18" x2="20" y2="18"/></svg>
           </button>
           <button onClick={() => addNewNode(NodeType.NOTE)} className="p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-all" title="Add Note">
             <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
           </button>
           <button onClick={() => addNewNode(NodeType.CHAT)} className="p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-all" title="Add Chat">
             <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
           </button>
           {onOpenStorage && (
             <button onClick={onOpenStorage} className={`p-2 rounded-lg transition-all ${storageConnected ? 'text-green-400' : 'text-slate-400 hover:text-white'}`}>
        ...
        <div className="flex flex-row md:flex-col gap-2 md:gap-4 border-l md:border-l-0 md:border-t border-slate-800 pl-2 md:pl-0 md:pt-4">
           <button onClick={() => applyLayout('tree-tb')} className="p-2 text-slate-500 hover:text-sky-400 hover:bg-slate-800 rounded-lg"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="5" r="2"/><circle cx="5" cy="19" r="2"/><circle cx="19" cy="19" r="2"/><line x1="12" y1="7" x2="5" y2="17"/><line x1="12" y1="7" x2="19" y2="17"/></svg></button>
           <button onClick={() => applyLayout('tree-lr')} className="p-2 text-slate-500 hover:text-sky-400 hover:bg-slate-800 rounded-lg"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" transform="rotate(-90 12 12)"><circle cx="12" cy="5" r="2"/><circle cx="5" cy="19" r="2"/><circle cx="19" cy="19" r="2"/><line x1="12" y1="7" x2="5" y2="17"/><line x1="12" y1="7" x2="19" y2="17"/></svg></button>
           <button onClick={() => applyLayout('force')} className="p-2 text-slate-500 hover:text-sky-400 hover:bg-slate-800 rounded-lg"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg></button>
        </div>
      </div>

      <div 
        ref={containerRef} 
        className={`flex-1 relative overflow-hidden cursor-default canvas-background min-w-0 ${connectingNodeId ? 'cursor-crosshair' : ''}`}
        style={{
      ...
      >
        ...
        >
          <defs>
             ...
             <marker id="arrowhead-active" markerWidth="10" markerHeight="7" refX="10" refY="3.5" orient="auto">
               <polygon points="0 0, 10 3.5, 0 7" fill={COLORS.activeEdgeStroke} />
             </marker>
          </defs>
           <g transform={`translate(${viewTransform.x},${viewTransform.y}) scale(${viewTransform.k})`}>
             ...
             {expandingNodeIds.map(id => {
               const node = nodes.find(n => n.id === id);
               if (!node) return null;
               return <SkeletonGraph key={`skeleton-${id}`} x={node.x + (node.width||300) + 50} y={node.y} />;
             })}
             {connectingLine}
           </g>
        </svg>
        ...
        >
             ...
             {visibleNodes.map(node => (
                <div key={`wrapper-${node.id}`} className="pointer-events-auto">
                    ...
                    key={node.id}
                    node={node}
                    isSelected={selectedNodeId === node.id || connectingNodeId === node.id}
        ...
        <div className="absolute bottom-6 left-6 pointer-events-none opacity-50 text-xs text-slate-500 font-mono">
        ...
        {expandingNodeIds.length > 0 && (
          <div className="absolute bottom-6 right-6 bg-slate-800/90 backdrop-blur text-sky-400 px-4 py-2 rounded-full border border-sky-500/30 shadow-lg animate-pulse flex items-center gap-2 z-[100]">
             <div className="w-2 h-2 bg-sky-400 rounded-full animate-bounce" />
             <span className="text-xs font-bold uppercase tracking-wide">Generating Graph ({expandingNodeIds.length})...</span>
          </div>
        )}
        ...
        {selectionTooltip && !connectingNodeId && (
           <div 
              ...
              style={{ 
                  ...
                  top: isMobile ? (selectionTooltip.bottom ?? selectionTooltip.y) : selectionTooltip.y, 
                  transform: isMobile ? 'translate(-50%, 10px)' : 'translate(-50%, -120%)' 
              }}
           ...
           >
              <button 
                 ...
                 onClick={(e) => { e.stopPropagation(); handleCreateFromSelection(NodeType.NOTE); }}
                 title={selectionTooltip.sourceId ? "Create Connected Note" : "Create Note"}
              >
              ...
              <button 
                 ...
                 onClick={(e) => { e.stopPropagation(); handleCreateFromSelection(NodeType.CHAT); }}
                 title={selectionTooltip.sourceId ? "Create Connected Chat" : "Create Chat"}
              >
                 <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
              ...
              {selectionTooltip.sourceId && (
                <>
                  ...
                  <button 
                     ...
                     onClick={(e) => { e.stopPropagation(); onExpandNode(selectionTooltip.sourceId!, selectionTooltip.text); setSelectionTooltip(null); window.getSelection()?.removeAllRanges(); }}
                     title="Expand Graph from selection"
                  >
                     <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3" /><circle cx="6" cy="6" r="2" /><circle cx="18" cy="6" r="2" /><line x1="10" y1="10" x2="7.5" y2="7.5" /><line x1="14" y1="10" x2="16.5" y2="7.5" /></svg>
