...
export const SearchBar: React.FC<SearchBarProps> = ({ onSelect }) => {
    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `https://api.wikimedia.org/core/v1/wikipedia/en/search/title?q=${encodeURIComponent(query)}&limit=6`
        );
        const data = await response.json();
        
        if (data.pages && data.pages.length > 0) {
           setResults(data.pages.map((p: any) => ({
             title: p.title,
  ...
  if (isMobile && !mobileExpanded) {
      return (
          <button
            className="absolute z-50 top-16 right-4 p-3 bg-slate-800 border border-slate-700 rounded-full shadow-xl text-slate-200 hover:text-white hover:bg-slate-700 transition-all"
            onClick={() => setMobileExpanded(true)}
            title="Search Wikipedia"
          >
              <svg className="h-6 w-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          </button>
      );
  }
      ...
      <div className="relative group font-sans">
         <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            {loading ? (
                ...
                <svg className="h-4 w-4 text-slate-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
            )}
         </div>
         <input 
            ref={inputRef}
         ...
         {isOpen && query && (
           <ul className="absolute mt-2 w-full bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600">
             {results.length > 0 ? results.map((result) => (
               <li key={result.title} className="flex border-b border-slate-700/50 last:border-0 hover:bg-slate-700/30 transition-colors group/item">
                 ...
                 >
                   {result.thumbnail?.url ? (
                     <img src={result.thumbnail.url} alt="" className="w-10 h-10 rounded-md object-cover bg-slate-700 shrink-0" />
                   ) : (
                ...
                <li className="p-2">
                    ...
                    >
                         ...
                         <div className="min-w-0">
                            <div className="font-medium text-emerald-400 group-hover:text-emerald-300 transition-colors">Chat with AI about "{query}"</div>
                            <div className="text-xs text-slate-500">Topic not found on Wikipedia</div>
                         </div>
                    </button>
                </li>
             )}
