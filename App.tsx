...
import { GraphEdge, GraphNode, NodeType, ViewportTransform, ChatMessage } from './types';
import { DEFAULT_NODE_HEIGHT, DEFAULT_NODE_WIDTH } from './constants';
import { pickDirectory, loadGraphFromDirectory, saveNodeToFile, deleteNodeFile, saveEdgesToFile, verifyPermission } from './services/storageService';
import { expandNodeTopic, sendChatMessage, getTopicSummaryPrompt, findRelationships } from './services/geminiService';
...
const parseTextToNodes = (text: string) => {
    ...
    const isList = listLines.length > 0 && listLines.length > (lines.filter(l => l.trim()).length * 0.3);
...
const App: React.FC = () => {
  ...
  const [autoGraphEnabled, setAutoGraphEnabled] = useState<boolean>(() => {
      try {
          ...
          if (saved) {
              const parsed = JSON.parse(saved);
              return parsed.autoGraphEnabled !== undefined ? !!parsed.autoGraphEnabled : true;
          }
      } catch(e) {}
      return true;
  });
  ...
  const [activeSidePane, setActiveSidePane] = useState<{ type: 'web' | 'node', data: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const potentialTargets = nodes
      .filter(n => n.id !== newNode.id && n.content.length > 3 && n.parentId === currentScopeId)
      .slice(-10);
  ...
  }, [nodes, edges, viewTransform, autoGraphEnabled, currentScopeId, selectedNodeId]);
  ...
  }, [activeSidePane, selectedNodeId]);

  const handleExpandNode = useCallback(async (id: string, topic: string, nodeOverride?: GraphNode, depth?: number) => {
    ...
    const depthToUse = depth !== undefined ? depth : (sourceNode.autoExpandDepth || 1);
    
    try {
      const isSelfExpansion = sourceNode.content.trim().toLowerCase() === topic.trim().toLowerCase();
      // Improved heuristic for local breakdown vs knowledge expansion
      const isList = /^\s*[-*•]|\d+\./m.test(topic);
      const isContentBreakdown = topic.length > 100 || topic.includes('\n') || isList;
      ...
      if (isContentBreakdown) {
          ...
          const stack = [{ indent: -1, id: parentNodeId, x: parentNodeX, y: parentNodeY }];
          ...
          subItems.forEach((item, i) => {
              ...
              stack.push({ indent: item.indent, id: newNodeId, x: newNodeX, y: newNodeY });
          });

      } else {
          // --- Gemini API Mode ---
          const existingNodeNames = nodes.filter(n => n.parentId === currentScopeId).map(n => n.content);
          ...
          if (result.nodes.length > 0) {
            ...
            const subNodes: GraphNode[] = result.nodes.map((n, i) => {
                ...
                const angle = startAngle + (i / result.nodes.length) * 2 * Math.PI;
            ...
            result.edges.forEach(e => {
                const targetSubNode = subNodes.find(sn => sn.content === e.targetName);
                const targetExistingNode = nodes.find(n => n.content === e.targetName && n.parentId === currentScopeId);
      ...
      if (depthToUse > 1 && nextNodesToExpand.length > 0) {
           ...
           Promise.all(nextNodesToExpand.map(node => 
  ...
  const handleSearchSelect = useCallback((topic: string, shouldExpand: boolean, isWiki: boolean = true) => {
    ...
    const newNode: GraphNode = {
      ...
      width: DEFAULT_NODE_WIDTH,
      height: DEFAULT_NODE_HEIGHT,
      link: isWiki ? `https://en.wikipedia.org/wiki/${encodeURIComponent(topic.replace(/ /g, '_'))}` : undefined,
  ...
  const handleNavigateUp = useCallback((exitingScopeId?: string) => {
  ...
  const getBreadcrumbs = () => {
      ...
      const combinedCrumbs: { id: string | null, name: string, type: string }[] = [{ id: null, name: 'Home', type: 'root' }, ...crumbs];
      ...
      if (selectedNodeId) {
          ...
          if (currentNodes.length > 0) {
              ...
              if (selectedNodeId === rootNode.id) {
                  combinedCrumbs.push({ id: rootNode.id, name: rootNode.content, type: 'node' });
              } else {
                  ...
                  if (found) {
                      ...
                      if (n) combinedCrumbs.push({ id: n.id, name: n.content, type: 'node' });
                  }
              }
          }
      }
  ...
  // Persistence logic omitted for brevity but should be here...
  const handleOpenStorage = async () => { /* ... existing ... */ };

  const sidebarNode = activeSidePane?.type === 'node' 
            ...
            {getBreadcrumbs().map((crumb, i) => (
                <React.Fragment key={i}>
                    ...
                    <button 
                        ...
                        className={`transition-colors pointer-events-auto ${crumb.id === selectedNodeId ? 'text-sky-400 font-bold cursor-default' : 'text-slate-400 hover:text-white'}`}
